...
parser.add_argument('--left', default='L.png', help='path to the left image')#1
parser.add_argument('--right', default='R.png', help='path to the right image')#2
parser.add_argument('--model-path', default='checkpoint_original/best_model.pt', help='path to the model')#3，model
parser.add_argument('--save-path', default='1.png', help='path to save the disp image')#4，存图路径
...
device_ids = [0]
device = torch.device('cuda:{}'.format(device_ids[0]))#5，cpu
device = torch.device('cpu')#5，gpu
...
def main():
    ...
    transform = T.Compose([Normalize(mean, std), ToTensor(), Pad(384, 464)])#6,input pad
    ...
    plt.figure(figsize=(4.64, 3.84))#7，存图尺寸
    plt.axis('off')
    plt.imshow(disp)
    plt.colorbar()
