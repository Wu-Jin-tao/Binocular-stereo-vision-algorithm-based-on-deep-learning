...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv4 = StackedBlocks(n_blocks=3, in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=4, dilation=4)  # new_size <= dilation*(kernel_size-1)+1
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, inputs):
        out = self.net(inputs)
        if self.downsample:
            inputs = self.downsample(inputs)#downsample时，identity mapping需要进行1*1卷积
        out = out + inputs
...
class Conv2dBn(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, use_relu=True):
        ...
        self.net = nn.Sequential(*net)  #why use *???
